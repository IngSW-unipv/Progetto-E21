...
public class WelcomeServletModerator extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if (req.getPathInfo().equals("/productDetails")) {
			...
			try {
				resp.getWriter().write(Rythm.render("productDetailsModerator.html", cookie, auctionHouse.getAuction(req.getParameter("auctionID")), auctionHouse.getProfile(req.getParameter("auctioner")).getImg(), ""));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
