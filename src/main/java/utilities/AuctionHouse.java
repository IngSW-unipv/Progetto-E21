		   ...
		   try {
			...
			// CREAZIONE AUCTIONS
			   ...
			   while(rs1.next() == true) { 
				  ...
				  if(currentDate.isBefore(eDate) && currentDate.isAfter(sDate)) { //Controllo se l'asta � scaduta
					  a1 = new Auction(rs1.getString("auctionID"));
					  auctions.add(a1);
				   }
				  else { //Se l'asta � scaduta la imposto come chiusa
...
//Metodo che tratta i diversi step del pagamento
	public int paymentNextStep(int cookie, String auctionID) throws Exception {
		...
		else if (a.getStatus().equals("paid")) //terzo step- Possibilit� di scrivere recensione
		{
			return 1;
		}
