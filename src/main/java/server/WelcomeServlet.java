	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if (req.getPathInfo().equals("/registerRequest")) {					//Gestione richiesta pagina di registrazione
			...
			try {
				...
				resp.getWriter().write(Rythm.render("productDetails.html", cookie, auctionHouse.getAuction(req.getParameter("auctionID")), auctionHouse.getParticipant(req.getParameter("auctioner")).getImg(), ""));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
			...
			try {
				...
				resp.getWriter().write(Rythm.render("productDetails.html", cookie, auctionHouse.getAuction(req.getParameter("auctionID")), auctionHouse.getParticipant(req.getParameter("auctioner")).getImg(), msg));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
			...
			int cookie = Integer.parseInt(req.getParameter("cookie"));
			try {
				resp.getWriter().write(Rythm.render("profile.html", cookie, auctionHouse.getParticipant(req.getParameter("profile")), auctionHouse.getReviews(req.getParameter("profile"))));
			} catch (Exception e) {
				e.printStackTrace();
			} 
		}
			...
			try {
				resp.getWriter().write(Rythm.render("myProfile.html", cookie, auctionHouse.getParticipant(pr), auctionHouse.getReviews(pr)));
			} catch (Exception e) {
				e.printStackTrace();
			} 
		}
			...
			try {
				Participant sender = auctionHouse.getParticipant(senderUsername);
				Participant receiver = auctionHouse.getParticipant(receiverUsername);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// Metodo per inviare una review all'utente
			...
			try {
				...
				resp.getWriter().write(Rythm.render("profile.html", cookie, auctionHouse.getParticipant(req.getParameter("username")),
		...
		//SEND MESSAGE
			...
			try {
				...
				resp.getWriter().write(Rythm.render("chat.html", cookie, auctionHouse.getUsername(cookie),
						receiverUsername, auctionHouse.getMessages(cookie, receiverUsername), auctionHouse.getParticipant(auctionHouse.getUsername(cookie)),
						auctionHouse.getParticipant(receiverUsername)));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				resp.getWriter().write(Rythm.render("error.html", cookie, e.getMessage()));
			}
		...
		//metodo per aggiornamento carta di credito
			...
			try {
				...
				auctionHouse.saveCreditCard(card);
				resp.getWriter().write(Rythm.render("editProfile.html", cookie, "", "Updated", "", "" ));
					...
					try {
						...
						for (int i=0; i< pendingAucton.size(); i++)
						{
							if( pendingAucton.get(i).getId() == cookie) {
								...
								resp.getWriter().write(Rythm.render("productDetails.html", cookie, auctionHouse.getAuction(auctionID), auctionHouse.getParticipant(auctionHouse.getUsername(cookie)).getImg()));
